...
#centered(column(
  ...
  down(),
  row(
    code-node("src/", "起動・CLI・設定", "レイアウト復元"),
    column(
      accent-node("crates/"),
      down(),
      row(
        column(
          code-node("client/", "Win32 ウィンドウ", "GDI 描画・IME", "レイアウト UI"),
          code-node("server/", "PTY ライフサイクル", "セッション階層", "IPC サーバー"),
          code-node("terminal/", "VT パーサ", "Grid", "文字幅計算", "ConPTY ラッパー"),
        ),
        column(
          code-node("protocol/", "ClientMessage", "ServerMessage", "共通型"),
          code-node("renderer/", "将来拡張を見据えた", "レンダラー用 crate"),
        ),
      ),
    ),
  ),
